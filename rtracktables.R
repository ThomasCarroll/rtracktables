...
mm9Genes <- read.delim("Downloads/mm9Genes_May2012.txt",sep="\t",h=T)
...
lines(nuc6h[[4]][1:3000,1],runmean(nuc6h[[4]][1:3000,2],1)-2.8,col="darkblue")
  ...
  tssRangesPos <- GRanges(seqnames(testRangesPos),IRanges(start(testRangesPos)-distanceOutTSS,start(testRangesPos)+distanceInTSS),name=testRangesPos$name)
  tssRangesNeg <- GRanges(seqnames(testRangesNeg),IRanges(end(testRangesNeg)-distanceInTSS,end(testRangesNeg)+distanceOutTSS),name=testRangesNeg$name)
  ttsRangesPos <- GRanges(seqnames(testRangesPos),IRanges(end(testRangesPos)-distanceInTTS,end(testRangesPos)+distanceOutTSS),name=testRangesPos$name)
  ttsRangesNeg <- GRanges(seqnames(testRangesNeg),IRanges(start(testRangesNeg)-distanceOutTTS,start(testRangesNeg)+distanceInTSS),name=testRangesNeg$name)
  
  testRangesPos <- GRanges(seqnames(testRangesPos),IRanges(start(testRangesPos)+distanceInTSS,end(testRangesPos)-distanceInTTS),name=testRangesPos$name)
  testRangesNeg <- GRanges(seqnames(testRangesNeg),IRanges(start(testRangesNeg)+distanceInTTS,end(testRangesNeg)-distanceInTSS),name=testRangesNeg$name)
  
 
    #
    #tempPosTSSMat <- matrix(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]])]),ncol=mean(width(tssRangesPos)),byrow=TRUE)
    #tempPosTTSMat <- matrix(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]])]),ncol=mean(width(ttsRangesPos)),byrow=TRUE)
    #tempNegTSSMat <- matrix(rev(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]])])),ncol=mean(width(tssRangesNeg)),byrow=TRUE)
    #tempNegTTSMat <- matrix(rev(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]])])),ncol=mean(width(ttsRangesNeg)),byrow=TRUE)
    #tempPosTSSMat <- matrix(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]])))),ncol=mean(width(tssRangesPos)),byrow=TRUE)
    #tempNegTSSMat <- matrix(rev(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]]))))),ncol=mean(width(tssRangesNeg)),byrow=TRUE)
    #tempPosTTSMat <- matrix(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]])))),ncol=mean(width(ttsRangesPos)),byrow=TRUE)
    #tempNegTTSMat <- matrix(rev(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]]))))),ncol=mean(width(ttsRangesNeg)),byrow=TRUE)
    
    #print("done1")
    if(length(tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]]) > 0){
      print("done1.1")
      #tempPosTSSMat <- matrix(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]])))),ncol=mean(width(tssRangesPos)),byrow=TRUE)
      #print("done1.2")
      tempPosTSSMat <- matrix(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]])]),ncol=mean(width(tssRangesPos)),byrow=TRUE)
      rownames(tempPosTSSMat) <- tssRangesPos[seqnames(tssRangesPos) %in% chromosomes[c]]$name
      #      print("done1.3")
    }
    #print("done2")
    if(length(ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]]) > 0){  
      #tempPosTTSMat <- matrix(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]])))),ncol=mean(width(ttsRangesPos)),byrow=TRUE)
      tempPosTTSMat <- matrix(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]])]),ncol=mean(width(ttsRangesPos)),byrow=TRUE)
      rownames(tempPosTTSMat) <- ttsRangesPos[seqnames(ttsRangesPos) %in% chromosomes[c]]$name
    }
    #print("done3")
    if(length(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]]) > 0){
      #tempNegTSSMat <- matrix(rev(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]]))))),ncol=mean(width(tssRangesNeg)),byrow=TRUE)  
      tempNegTSSMat <- matrix(rev(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]])])),ncol=mean(width(tssRangesNeg)),byrow=TRUE)
      rownames(tempNegTSSMat) <- rev(tssRangesNeg[seqnames(tssRangesNeg) %in% chromosomes[c]]$name)
    }
    #print("done4")  
    if(length(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]]) > 0){
      #tempNegTTSMat <- matrix(rev(as.vector(unlist(Views(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]],ranges(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]]))))),ncol=mean(width(ttsRangesNeg)),byrow=TRUE)
      tempNegTTSMat <- matrix(rev(as.vector(genomeCov[[which(names(genomeCov) %in% chromosomes[c])]][ranges(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]])])),ncol=mean(width(ttsRangesNeg)),byrow=TRUE)
      rownames(tempNegTTSMat) <- rev(ttsRangesNeg[seqnames(ttsRangesNeg) %in% chromosomes[c]]$name)
    }
    #print("done5")
    PosTSSmat <- rbind(PosTSSmat,tempPosTSSMat)
    PosTTSmat <- rbind(PosTTSmat,tempPosTTSMat)
    negTSSmat <- rbind(negTSSmat,tempNegTSSMat)
    negTTSmat <- rbind(negTTSmat,tempNegTTSMat)
    tempPosTSSMat <- NULL
    tempNegTSSMat <- NULL  
    tempPosTTSMat <- NULL
    tempNegTTSMat <- NULL    
  ...
  AllTSSmat <- rbind(PosTSSmat,negTSSmat)
  cat("Done TSSs")
  AllTTSmat <- rbind(PosTTSmat,negTTSmat)
  cat("Done TTS")
  meansMat <- matrix(meansList,ncol=nOfWindows,byrow=T)
  cat("Done Matrix")
  rownames(meansMat) <- matrix(names(meansList),ncol=nOfWindows,byrow=T)[,1]
  ...
  return(list(meansMat,AllTSSmat,AllTTSmat,list(totalReads,length(total))))
}
